...
def public_paths_labeled(root):
    ...
    labels_raw = glob.glob(os.path.join(root, "Public/labels/*"))

    labels_raw = sorted([p.replace("_label", "") for p in labels_raw])
    labels_raw = [p.replace(".tiff", "_label.tiff") for p in labels_raw]

    data_dicts = []

    for i, (image_path, label_path) in enumerate(zip(images_raw, labels_raw)):
